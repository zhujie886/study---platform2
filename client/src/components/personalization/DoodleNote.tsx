...
export const DoodleNote = ({
  ...
  const send = async () => {
    ...
    if (!hasInk && !message.trim()) {
      toast.error("先画点内容或写一句话再发送");
      return;
    }

    setIsSending(true);
    const toastId = toast.loading("正在发送…");
    ...
    >
      ...
      <div className="p-4 pt-7 flex justify-between items-center z-10">
        <div className="flex flex-col">
          <span className="text-yellow-800/60 text-xs font-bold tracking-widest uppercase">From: You</span>
          <span className="text-slate-700 text-sm font-semibold leading-tight">小纸条涂鸦</span>
        </div>
        <button
      ...
      <div className="px-4 pb-3">
        ...
        <div className="mt-2 grid grid-cols-2 gap-2">
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs text-slate-500 w-8">粗细</span>
            <input
              type="range"
              min={2}
              max={14}
