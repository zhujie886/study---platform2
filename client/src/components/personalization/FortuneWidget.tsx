...
// --- 类型定义 ---
type FortuneLevel = '上上' | '上中' | '中' | '下中' | '下下';
...
export const FortuneWidget = () => {
  ...
  const drawFortune = () => {
    ...
    setTimeout(() => {
      ...
      if (roll <= 25) level = '上上';
      else if (roll <= 50) level = '上中';
      else if (roll <= 75) level = '中';
      else if (roll <= 100) level = '下中';
      else level = '下下';
    ...
    <div className="w-72 relative group select-none font-sans">
      ...
      <div className="relative bg-[#0f0c29]/90 backdrop-blur-xl rounded-[2.5rem] border border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.3)] overflow-hidden flex flex-col items-center p-6">
        ...
        <div className="relative z-10 flex items-center gap-2 mb-8">
          ...
          <span className="text-sm font-medium text-transparent bg-clip-text bg-gradient-to-r from-purple-200 via-pink-200 to-blue-200 tracking-[0.2em] shadow-sm">命运星盘</span>
          <SparklesIcon className="w-4 h-4 text-purple-300/80 animate-pulse" />
        </div>
              ...
              >
                ...
                >
                  ...
                  <div className="relative bg-slate-900/80 backdrop-blur-md rounded-2xl p-5 flex flex-col items-center text-center border border-white/10">
                    ...
                    <h4 className="text-sm font-bold text-white/90 mb-3">{result.title}</h4>
                    <p className="text-xs leading-relaxed text-white/70 font-serif italic px-1">“{result.poem}”</p>
                  </div>
