...
export const TimeCapsule = ({ onClose }: { onClose: () => void }) => {
  ...
  const seal = () => {
    if (!content) return;
    setStep('locked');
    setUnlockTime(new Date(Date.now() + 3 * 24 * 60 * 60 * 1000).toLocaleDateString());
  };
  ...
  return (
    <div className="w-80 bg-gradient-to-br from-violet-50 to-fuchsia-50 rounded-[30px] shadow-2xl p-6 relative border border-white flex flex-col min-h-[400px]">
      <button
        ...
        aria-label={t('timecapsule.close')}
      >
        &times;
      </button>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex-1 flex flex-col">
          <div className="relative flex-1 mb-4">
            ...
            <div className="absolute bottom-3 right-3 text-xs text-purple-300">
              {t('timecapsule.count', { count: content.length })}
            </div>
          </div>
          ...
          <div className="flex justify-between items-center text-xs text-purple-500 mb-4 bg-purple-100/50 p-2 rounded-lg">
            <span className="flex items-center gap-1">
              ...
              {t('timecapsule.unlock_time')}
            </span>
            <span className="font-bold">{t('timecapsule.unlock_in')}</span>
          </div>

          <button
            onClick={seal}
            className="w-full bg-gradient-to-r from-purple-400 to-fuchsia-400 text-white py-3 rounded-xl font-bold shadow-md hover:shadow-lg hover:scale-105 transition-all active:scale-95"
          >
            {t('timecapsule.seal_button')} &#128220;
          </button>
        </motion.div>
      )}
      ...
      {step === 'locked' && (
        ...
        >
          ...
          <h3 className="text-lg text-purple-800 font-bold mb-2">{t('timecapsule.sealed_title')}</h3>
          <p className="text-xs text-purple-400 leading-5 px-4">
            {t('timecapsule.sealed_wait')}
            <br />
            {t('timecapsule.sealed_open_at', { date: unlockTime })}
          </p>
          <button
            onClick={() => setStep('open')}
            className="mt-8 text-[10px] text-slate-300 hover:text-purple-400 underline"
          >
            {t('timecapsule.force_open')}
          </button>
        </motion.div>
      )}
