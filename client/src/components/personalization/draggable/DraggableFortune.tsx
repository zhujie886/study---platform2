...
import { getFortunePool } from "@/utils/fortunePool";
...
export const DraggableFortune: React.FC<Props> = ({
  ...
  const pool = useMemo(() => getFortunePool(lang), [lang]);

  const drawFortune = () => {
    const pick = pool[Math.floor(Math.random() * pool.length)];
    const next = `${pick.label} ? ${pick.text}`;
    setFortune(next);
    onRefresh?.(next);
  };
        ...
        <div className="flex items-center gap-1">
          ...
          {onClose && (
            <button
              ...
              title={t("fortune.draggable.close")}
            >
              ?
            </button>
          )}
