...
export default function ConsultantProfilePage() {
  ...
  if (!profile) return <div className="text-center py-16">未找到咨询师</div>;
    ...
    <div className="max-w-5xl mx-auto px-6 py-8 space-y-8">
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex flex-col md:flex-row gap-6">
        ...
        <div className="flex-1">
          <div className="flex flex-wrap items-center gap-3">
            ...
            {profile.isVerified && (
              <span className="px-2 py-1 text-xs bg-green-100 text-green-700 rounded-full">已认证</span>
            )}
            {profile.isConsultant && (
              <span className="px-2 py-1 text-xs bg-blue-100 text-blue-700 rounded-full">咨询师</span>
            )}
          </div>
          ...
          <div className="flex flex-wrap gap-3 text-sm text-gray-600 mt-4">
            {normalized.profileInfo.primaryDomain && (
              <span>主领域：{normalized.profileInfo.primaryDomain}</span>
            )}
            {normalized.secondaryDomains.length > 0 && (
              <span>辅领域：{normalized.secondaryDomains.join(' / ')}</span>
            )}
            {normalized.profileInfo.timezone && <span>时区：{normalized.profileInfo.timezone}</span>}
            {normalized.languages.length > 0 && <span>语言：{normalized.languages.join(' / ')}</span>}
          </div>

          <div className="flex flex-wrap gap-6 text-sm text-gray-600 mt-4">
            <span>
              评分：{ratingAvg ? ratingAvg.toFixed(1) : '暂无'} {ratingAvg > 0 && '⭐'}
            </span>
            <span>评价数：{ratingCount}</span>
            <span>预约次数：{profile.totalBookings ?? 0}</span>
          </div>
      ...
      {(normalized.portfolioLinks.length > 0 || normalized.projects.length > 0 || normalized.certifications.length > 0) && (
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-6">
          {normalized.portfolioLinks.length > 0 && (
            <div>
              <h2 className="text-lg font-semibold mb-3">作品集与链接</h2>
          ...
          {normalized.projects.length > 0 && (
            <div>
              <h2 className="text-lg font-semibold mb-3">项目展示</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {normalized.projects.map((item: any, index: number) => {
                  const title = typeof item === 'string' ? item : item.title || item.name || '项目';
                  const description =
          ...
          {normalized.certifications.length > 0 && (
            <div>
              <h2 className="text-lg font-semibold mb-3">证书与资质</h2>
              <div className="flex flex-wrap gap-2">
                {normalized.certifications.map((item: any, index: number) => {
                  const label = typeof item === 'string' ? item : item.title || item.name || item.label || '证书';
                  return (
                    <span
                      key={`${label}-${index}`}
      ...
      {normalized.availabilityRules.length > 0 && (
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
          <h2 className="text-lg font-semibold mb-3">可预约时间</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm text-gray-600">
            {normalized.availabilityRules.map((rule: any) => (
              <div key={rule.id} className="flex items-center justify-between border rounded-lg px-3 py-2">
                <span>{dayLabels[rule.dayOfWeek] || '周?'}</span>
                <span>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        <h2 className="text-lg font-semibold mb-4">服务套餐</h2>
        {profile.services?.length ? (
            ...
            {profile.services.map((service: any) => (
              <div key={service.id} className="border border-gray-100 p-5 rounded-xl bg-white">
                ...
                <p className="text-gray-500 text-sm mt-2">
                  {service.durationMinutes} 分钟 · {service.deliveryType}
                </p>
                ...
                {(service.scope || service.deliverables || service.notes) && (
                  <div className="text-sm text-gray-600 mt-3 space-y-1">
                    {service.scope && <div>适用范围：{service.scope}</div>}
                    {service.deliverables && <div>交付物：{service.deliverables}</div>}
                    {service.notes && <div>购买须知：{service.notes}</div>}
                  </div>
                )}
          ...
          <div className="text-gray-500 text-sm">暂无可预约服务</div>
        )}
      </div>
    </div>
  );
