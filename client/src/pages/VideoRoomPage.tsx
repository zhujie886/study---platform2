...
export default function VideoRoomPage() {
  ...
  const [messages, setMessages] = useState<{user: string, text: string, time: string, isSelf: boolean}[]>([]);
  ...
  useEffect(() => {
    const timer = setInterval(
      () => setCurrentTime(new Date().toLocaleTimeString(lang === 'en' ? 'en-US' : 'zh-CN')),
      1000
    );
  ...
  const togglePiP = async () => {
    ...
    } catch (error) { showToast(t('不支持画中画')); }
  };
  ...
  const sendMessage = () => {
    ...
    setTimeout(() => {
      setMessages(prev => [...prev, {
        user: t('助手'),
        text: t('收到: {text}', { text: newMsg.text }),
        time: new Date().toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}),
        isSelf: false
      }]);
    }, 1000);
  ...
  if (!user) return <div className="h-screen w-full bg-gray-900 flex items-center justify-center text-white">{t('加载中...')}</div>;
    ...
    <div className="flex h-screen w-full bg-gray-900 text-white overflow-hidden relative">
      ...
      <div className={`flex flex-col flex-1 h-full relative transition-all duration-300 ${showChat ? 'mr-80' : 'mr-0'}`}>
        ...
        <div className="absolute top-4 right-4 z-20 flex gap-2">
          ...
          <button onClick={() => {navigator.clipboard.writeText(window.location.href); showToast(t('链接已复制'))}} className="p-2 bg-indigo-600 rounded-lg shadow-lg"><Share2 size={18} /></button>
        </div>
        ...
        <div className="flex-1 min-h-0 w-full relative bg-gray-800 p-4">
          <div className={`h-full w-full flex flex-col gap-4 ${showWhiteboard ? 'lg:flex-row' : ''}`}>
            ...
            <div className="flex-1 min-h-0 relative bg-gray-900/40 rounded-2xl p-3 flex items-center justify-center">
              ...
              <div className="absolute bottom-4 left-4 bg-black/60 px-3 py-1 rounded text-sm font-medium">{user.username} ({t('我')})</div>
            </div>
          </div>
        </div>
        ...
        <div className="h-20 flex-shrink-0 bg-gray-900 border-t border-gray-800 flex items-center justify-center gap-3 px-4 z-50 select-none">
          <ControlButton active={isMuted} onClick={() => setIsMuted(!isMuted)} onIcon={<MicOff />} offIcon={<Mic />} label={t('静音')} color="red" />
          <ControlButton active={isVideoOff} onClick={() => setIsVideoOff(!isVideoOff)} onIcon={<VideoOff />} offIcon={<Video />} label={t('视频')} color="red" />
          <div className="w-px h-8 bg-gray-700 mx-2"></div>
          <ControlButton active={isScreenSharing} onClick={toggleScreenShare} onIcon={<MonitorUp />} offIcon={<MonitorUp />} label={t('共享')} color="green" />
          <ControlButton active={showWhiteboard} onClick={() => setShowWhiteboard(!showWhiteboard)} onIcon={<PenTool />} offIcon={<PenTool />} label={t('白板')} color="indigo" />
          <ControlButton active={isRecording} onClick={toggleRecording} onIcon={<Download />} offIcon={<Circle className="fill-current" />} label={t('录制')} color="red" />
          <ControlButton active={isCaptionsEnabled} onClick={() => setIsCaptionsEnabled(!isCaptionsEnabled)} onIcon={<FileText />} offIcon={<FileText />} label={t('字幕')} color="indigo" />
          <div className="w-px h-8 bg-gray-700 mx-2"></div>

          <div className="relative">
             <ControlButton active={showReactions} onClick={() => setShowReactions(!showReactions)} onIcon={<Smile />} offIcon={<Smile />} label={t('反应')} color="yellow" />
             {showReactions && (
          ...
          <ControlButton active={isHandRaised} onClick={() => { setIsHandRaised(!isHandRaised); if(!isHandRaised) showToast(t('已举手')); }} onIcon={<Hand />} offIcon={<Hand />} label={t('举手')} color="yellow" />
          <ControlButton active={showInfo} onClick={() => setShowInfo(!showInfo)} onIcon={<Info className="fill-current" />} offIcon={<Info />} label={t('信息')} color="indigo" /> {/* ★★★★★ 核心修复：新增信息按钮 */}
          <div className="w-px h-8 bg-gray-700 mx-2"></div>
          <ControlButton active={showChat} onClick={() => setShowChat(!showChat)} onIcon={<MessageSquare className="fill-current" />} offIcon={<MessageSquare />} label={t('聊天')} color="indigo" />
      ...
      <div className={`absolute top-0 right-0 h-full w-80 bg-gray-800 border-l border-gray-700 shadow-2xl z-[55] transition-transform duration-300 ${showChat ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="flex flex-col h-full">
          <div className="p-4 border-b border-gray-700 flex justify-between items-center bg-gray-900/50">
            <h3 className="font-semibold flex items-center gap-2">{t('聊天室')}</h3>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
             {messages.map((msg, i) => (
              <div key={i} className={`flex flex-col ${msg.isSelf ? 'items-end' : 'items-start'}`}>
                 ...
                 <div className={`px-3 py-2 rounded-lg max-w-[85%] text-sm ${msg.isSelf ? 'bg-indigo-600 text-white' : 'bg-gray-700'}`}>{msg.text}</div>
              </div>
            ))}
          </div>
          <div className="p-4 border-t border-gray-700 bg-gray-900/50 flex gap-2">
            <input type="text" value={inputMessage} onChange={e => setInputMessage(e.target.value)} placeholder={t('输入消息...')} className="flex-1 bg-gray-700 rounded-full px-4 py-2 text-sm text-white outline-none" />
    ...
    <div className="absolute inset-0 bg-black/60 flex items-center justify-center z-[60]">
      <div className="bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md border border-gray-700 animate-fade-in-up">
        <div className="p-6 border-b border-gray-700 flex justify-between items-center">
          <h3 className="font-semibold text-lg">{t('会议信息')}</h3>
          <button onClick={onClose} className="text-gray-400 hover:text-white"><X size={20} /></button>
        </div>
        <div className="p-6 space-y-4">
          <div>
            <label className="text-sm text-gray-400">{t('会议链接')}</label>
            <div className="flex items-center gap-2 mt-1">
              ...
              <button onClick={() => copyToClipboard(meetingLink, t('会议链接'))} className="p-2 bg-indigo-600 rounded-lg hover:bg-indigo-500"><Copy size={16} /></button>
            </div>
          </div>
          <div>
            <label className="text-sm text-gray-400">{t('会议号')}</label>
            <div className="flex items-center gap-2 mt-1">
              <input type="text" readOnly value={roomId} className="flex-1 bg-gray-900 rounded-lg px-3 py-2 text-sm border border-gray-700" />
              <button onClick={() => copyToClipboard(roomId, t('会议号'))} className="p-2 bg-indigo-600 rounded-lg hover:bg-indigo-500"><Copy size={16} /></button>
            </div>
          </div>
        </div>
      </div>
