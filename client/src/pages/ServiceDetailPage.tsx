...
export default function ServiceDetailPage() {
  ...
  if (loading) {
    return <div className="text-center py-20">正在加载服务...</div>;
  }
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="p-8 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm text-gray-600">
              ...
              <div className="border border-gray-100 rounded-xl p-4">
                <div className="text-gray-400 text-xs">交付方式</div>
                <div className="text-lg font-semibold text-gray-900 mt-2">{service.deliveryType}</div>
              </div>
              <div className="border border-gray-100 rounded-xl p-4">
                <div className="text-gray-400 text-xs">服务价格</div>
            ...
            {(service.scope || service.deliverables || service.notes) && (
              <div className="space-y-3 text-sm text-gray-600">
                {service.scope && (
                  <div>
                    <span className="font-medium text-gray-800">适用范围：</span>
                    {service.scope}
                  </div>
                )}
                {service.deliverables && (
                  <div>
                    <span className="font-medium text-gray-800">交付物：</span>
                    {service.deliverables}
                  </div>
                )}
                {service.notes && (
                  <div>
                    <span className="font-medium text-gray-800">购买须知：</span>
                    {service.notes}
                  </div>
                )}
              </div>
