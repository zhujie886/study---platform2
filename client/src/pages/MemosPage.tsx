...
export default function MemosPage() {
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm(t('确定删除吗？'))) return;
    try {
    ...
    <div className="space-y-6 animate-fade-in-up">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold text-gray-800 flex items-center gap-2">
          <DocumentTextIcon className="w-8 h-8 text-blue-500" />{t('我的备忘录')}
        </h1>
      </div>
      ...
      <form onSubmit={handleCreate} className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex gap-4">
        ...
        >
          <PlusIcon className="w-5 h-5" />{t('新建')}
        </button>
      </form>

      {loading ? (
        <div className="text-center py-10 text-gray-400">{t('加载中...')}</div>
      ) : memos.length === 0 ? (
        <div className="text-center py-20 bg-gray-50 rounded-2xl border border-dashed border-gray-200">
          <p className="text-gray-500">{t('还没有备忘录，试着创建一条吧！')}</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {memos.map((memo: any) => (
            <div key={memo.id} className="group bg-white p-6 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition relative">
              <h3 className="font-bold text-lg text-gray-800 mb-2 truncate">{memo.title || t('无标题')}</h3>
